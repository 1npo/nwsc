...
def parse_radar_station_data(radar_station_data: dict) -> dict:
	...
	if adaptation and isinstance(adaptation, dict):
		station.update({
			...
			'transmitter_power_data_watts_factor': 				adaptation.get('timestamp'),
			'antenna_gain_incl_radome': 						adaptation.get('timestamp'),
			'coho_power_at_a1j4': 								adaptation.get('timestamp'),
			'stalo_power_at_a1j2': 								adaptation.get('timestamp'),
			'horizontal_receiver_noise_long_pulse': 			adaptation.get('timestamp'),
			'horizontal_receiver_noise_short_pulse': 			adaptation.get('timestamp'),
			'transmitter_spectrum_filter_installed': 			adaptation.get('timestamp'),
			'pulse_width_transmitter_out_long_pulse': 			adaptation.get('timestamp'),
			'pulse_width_transmitter_out_short_pulse': 			adaptation.get('timestamp'),
			'ame_noise_source_horizontal_excess_noise_ratio': 	adaptation.get('timestamp'),
			'ame_horizontal_test_signal_power': 				adaptation.get('timestamp'),
			'wg04_circulator':									adaptation.get('timestamp'),
			'wg02_harmonic_filter':								adaptation.get('timestamp'),
			'wg06_spectrum_filter':								adaptation.get('timestamp'),
			'ifd_rif_anti_alias_filter':						adaptation.get('timestamp'),
			'ifd_burst_anti_alias_filter':						adaptation.get('timestamp'),
			'a6_arc_detector':									adaptation.get('timestamp'),
			'transmitter_coupler_coupling':						adaptation.get('timestamp'),
			'vertical_f_heliax_to_4at16':						adaptation.get('timestamp'),
			'horizontal_f_heliax_to_4at17':						adaptation.get('timestamp'),
			'at4_attenuator':									adaptation.get('timestamp'),
			'waveguide_klystron_to_switch':						adaptation.get('timestamp'),
		})
